...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      prop_op.props.clear();
      prop_op.props[ "subsidized_accounts_per_day" ] = fc::raw::pack_to_vector( 0x0000000100000000LL );
      STEEM_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "subsidized_accounts_per_day" ] = fc::raw::pack_to_vector( 1000 );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.subsidized_accounts_per_day == 1000 );
